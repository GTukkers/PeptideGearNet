'''
Title: Leave one out with GearNet-Edge embedding
'''
import resource
import pandas as pd
import torch as t
...
def train_validate_model2(neural_network, train_dataloader, validation_dataloader, test_dataloader, device, optimizer, loss_function, epochs, batch_size=None):
    ...
    del neural_network, loss_function, optimizer
...
def main():
    ...
    # Load dataframe, containing the IDs and the clusters
    cluster_df = pd.read_csv(param.loo_df)
    for i in range(5):
        i += 1
        if param.concat_gearnet:
            embedding = load_embedding(param.embedding_3072)
            print("use embedding 3072")
        else:
            embedding = load_embedding(param.embedding_512)
            print("use embedding 512")
    
        label = f"cluster{str(i)}"
        test_ids = cluster_df.loc[cluster_df['cluster_label'] == label, 'ID'].tolist()
        train_val_df = cluster_df[cluster_df['cluster_label'] != label]
        train_ids, validation_ids = split_data(train_val_df, train_val_df.ID.tolist())
        ...
        t.save(final_model, f"{param.output_dir}/{saved_model}_cluster_{i}.pth")
        print(f"Saved model in {param.output_dir}/{saved_model}_cluster_{i}.pth")
        t.save(best_model, f"{param.output_dir}/best_model_{saved_model}_cluster_{i}.pth")
        print(f"Saved best model of cluster {i} at epoch {best_model['epoch']} with loss {best_model['lowest_loss']}")
        del neural_network, loss_function, params, optimizer, final_model, best_model
        del train_dataloader, validation_dataloader, test_dataloader
