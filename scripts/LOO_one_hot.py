'''
Title: Leave One Out, one-hot encoding
'''
...
def main():
    ...
    for i in range(5):
        i += 1
        cluster_df = load_and_preprocess_dataframe(param.loo_df)
        label = f"cluster{str(i)}"

        test_df = cluster_df.loc[cluster_df['cluster_label'] == label]
        print(f"length of test df {len(test_df)}")
        train_val_df = cluster_df[cluster_df['cluster_label'] != label]
        print(f"length of train_val df {len(train_val_df)}")
        
        train_df, validation_df = split_data(train_val_df)
        ...
        torch.save(final_model, f"{param.output_dir}/{saved_model}_cluster_{i}.pth")
        print(f"Saved model in {param.output_dir}/{saved_model}_cluster_{i}.pth")
        torch.save(best_model, f"{param.output_dir}/best_model_{saved_model}_cluster_{i}.pth")
