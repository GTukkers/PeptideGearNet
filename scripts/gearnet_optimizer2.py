...
def create_data_loaders(dataset, batch_size, ntasks):
    return data.dataloader.DataLoader(
        ...
        num_workers=ntasks,
        drop_last=False, #Turn to false to make sure that I have al the ID
    )

# Define your training function
def train_epoch(task, train_dataloader, device, optimizer, loss_function, train_metrics, input_size, train_gearnet_weights=None, batch_size=None, dataframe=None):
    task.mlp.train()

    for (minib, batch) in enumerate(train_dataloader):
        graph = task.graph_construction_model(batch["graph"]).to(device)
        #target_wrong = batch["targets"].max(dim=1).indices.float().to(device).unsqueeze(1)
        complexes = batch['id']
...
def validate_epoch(task, validation_dataloader, device, loss_function, val_metrics, input_size, batch_size=None, dataframe=None):
    task.mlp.eval()
    epoch_val_loss = []
...
def train_model(task, train_dataloader, validation_dataloader, test_dataloader, device, optimizer, loss_function, epochs, input_size, train_gearnet_weights=None, batch_size=None, dataframe=None):
    best_model = {
        "model": None,
        "lowest_loss": 10000,
        "epoch": None,
    ...
    for epoch in range(epochs):
        ...
        t1 = time.time()
        train_epoch(task, train_dataloader, device, optimizer, loss_function, metrics["train"], input_size, train_gearnet_weights, batch_size, dataframe)
        epoch_valid_loss = validate_epoch(task, validation_dataloader, device, loss_function, metrics["val"], input_size, batch_size, dataframe)
...
def train_validate_model2(task, train_dataloader, validation_dataloader, test_dataloader, device, epochs, input_size, train_gearnet_weights=None, batch_size=None, dataframe=None, dataframe_test=None):
    metrics = {
        "train": {"ID": [], "embedding": [], "target": []},
        "val": {"ID": [], "embedding": [], "target": []},
        "test": {"ID": [], "embedding": [], "target": []},
    }
    ...
    for (minib, batch) in enumerate(train_dataloader):
        ...
        metrics["train"]["ID"].append(complexes)
        metrics["train"]["embedding"].append(peptide_emb_np)
        metrics["train"]["target"].append(target_np)

    df_train = pd.DataFrame(metrics['train'])
    ...
    for (minib, batch) in enumerate(validation_dataloader):
        ...
        metrics["val"]["ID"].append(complexes)
        metrics["val"]["embedding"].append(peptide_emb_np)
        metrics["val"]["target"].append(target_np)

    df_val = pd.DataFrame(metrics['val'])
    ...
    for (minib, batch) in enumerate(test_dataloader):
        ...
        metrics["test"]["ID"].append(complexes)
        metrics["test"]["embedding"].append(peptide_emb_np)
        metrics["test"]["target"].append(target_np)

    df_test = pd.DataFrame(metrics['test'])
...
def main():
    ...
    # Need to keep current train and validation set, seperate so that I can compare it with my current results
    ...
    df_train, df_val, df_test = train_validate_model2(task, train_dataloader, validation_dataloader, test_dataloader, device, param.epochs, input_size=input_size, train_gearnet_weights=param.with_gearnet_weights, batch_size=param.batch_size, dataframe=df, dataframe_test=df_test)
    saved_model = f"GearNet-Edge_embedding"
    df_train.to_pickle(f"{param.output_dir}/{saved_model}_train_512.pkl")
    df_val.to_pickle(f"{param.output_dir}/{saved_model}_val_512.pkl")
    df_test.to_pickle(f"{param.output_dir}/{saved_model}_test_512.pkl")
