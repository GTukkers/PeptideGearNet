...
from custom_dataset import ProteinDataset, MLP, Own_MultipleBinaryClassification
from finetune_args import param
import time
import warnings
from mlp_designs import MLP_after_encoder, Encoder, Autoencoder, EncoderModule
import sys
...
def split_data(df, ids):
    training_ids, test_ids = train_test_split(ids, test_size=0.2, stratify=df.binder, random_state=1)
    df_training = df[df.ID.isin(training_ids)]
    train_ids, validation_ids = train_test_split(training_ids, test_size=0.25, stratify=df_training.binder, random_state=1)
    print(f"len of train_ids: {len(train_ids)}\nlen of validation_ids: {len(validation_ids)}\nlen of test_ids: {len(test_ids)}")
...
def setup_task(gearnet_edge, graph_construction_model):
    task = Own_MultipleBinaryClassification(
        gearnet_edge,
        task=(0, 1),
    ...
    # Override the encoder MLP to use the one in your model
    #task.encoder_mlp = gearnet_edge.encoder_mlp  # Assuming 'encoder_mlp' is a property of gearnet_edge

    return task

# Function to load weights, map to device, and set model to eval mode
def prepare_task_model(task, ss_weights, device, own_mlp=None, encoder=None, encoder_weights=None):
    task.model.load_state_dict(ss_weights, strict=False)
    task.model.to(device)
    task.model.eval()
    task.encoder_mlp = encoder
    task.encoder_mlp.load_state_dict(encoder_weights)
    task.encoder_mlp.eval()
    task.encoder_mlp.to(device)
    task.mlp = own_mlp                                          
    task.mlp.to(device)
...
def train_epoch(task, train_dataloader, device, optimizer, loss_function, train_metrics, input_size, train_gearnet_weights=None, batch_size=None):
    task.mlp.train()
    for (minib, batch) in enumerate(train_dataloader):
        graph = task.graph_construction_model(batch["graph"]).to(device)
        target = batch["targets"].max(dim=1).indices.float().to(device).unsqueeze(1)
        ...
        encoded = task.encoder_mlp(peptide_emb)
        print(encoded[:10])
        predict = task.mlp(encoded)
        #print(predict[:10])
...
def validate_epoch(task, validation_dataloader, device, loss_function, val_metrics, input_size, batch_size=None):
    task.mlp.eval()
    epoch_val_loss = []
...
def train_model(task, train_dataloader, validation_dataloader, device, optimizer, loss_function, epochs, input_size, train_gearnet_weights=None, batch_size=None):
    best_model = {
        "model": None,
        "lowest_loss": 10000,
        "epoch": None
    ...
    for epoch in range(epochs):
        ...
        t1 = time.time()
        train_epoch(task, train_dataloader, device, optimizer, loss_function, metrics["train"], input_size, train_gearnet_weights, batch_size)
        epoch_valid_loss = validate_epoch(task, validation_dataloader, device, loss_function, metrics["val"], input_size, batch_size)
...
def train_validate_model2(task, train_dataloader, validation_dataloader, test_dataloader, device, optimizer, loss_function, epochs, input_size, train_gearnet_weights=None, batch_size=None, dataframe=None):
    best_model = {
        "model": None,
        "lowest_loss": 10000,
        "epoch": None,
...
def main():
    ...
    train_ids, validation_ids, test_ids = split_data(df, ids)
    ...
    autoencoder_state_dict = t.load(param.autoencoder, map_location=device)

    encoder_state_dict = {
    "encoder.1.weight": autoencoder_state_dict['model']['model.1.weight'],
    "encoder.1.bias": autoencoder_state_dict['model']['model.1.bias'],
    "encoder.2.weight": autoencoder_state_dict['model']['model.2.weight'],
    "encoder.2.bias": autoencoder_state_dict['model']['model.2.bias'],
    "encoder.2.running_mean": autoencoder_state_dict['model']['model.2.running_mean'],
    "encoder.2.running_var": autoencoder_state_dict['model']['model.2.running_var'],
    "encoder.2.num_batches_tracked": autoencoder_state_dict['model']['model.2.num_batches_tracked']}

    neural_network = MLP_after_encoder(input_shape=input_size, hidden1=param.hidden1, hidden2=param.hidden2, hidden3=param.hidden3, dropout_rate=param.dropout)
    neural_network_state_dict = t.load(param.last_model, map_location=device)
    neural_network.load_state_dict(neural_network_state_dict["model"])
    encoder = Encoder(input_shape=input_size, hidden1=param.hidden1, hidden2=param.hidden2, hidden3=param.hidden3, dropout_rate=param.dropout)
    print("it works")

    #neural_network = Autoencoder(input_shape=input_size, hidden1=param.hidden1, hidden2=param.hidden2, hidden3=param.hidden3, dropout_rate=param.dropout)
    prepare_task_model(task, ss_weights, device, neural_network, encoder, encoder_state_dict)
    print(task.encoder_mlp)
    print(task.mlp)
    ...
    # Is now going to change to only the parameters of the mlp
    optimizer = t.optim.Adam(params=params, lr=1e-4, weight_decay=1e-5)
    final_model, best_model = train_validate_model2(task, train_dataloader, validation_dataloader, test_dataloader, device, optimizer, loss_function, param.epochs, input_size=input_size, train_gearnet_weights=param.with_gearnet_weights, batch_size=param.batch_size, dataframe=df)
