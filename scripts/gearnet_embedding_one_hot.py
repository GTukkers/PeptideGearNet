...
class Embedding_Dataset(Dataset):
    def __init__(self, embeddings, targets, ids, onehot):
...
def create_data_loaders(df, dataset, batch_size, num_workers):
    ids = dataset.ID.tolist()
    df_small = df[df['ID'].isin(ids)]
    onehot = t.tensor(df_small['one_hot'].tolist())
    ...
    print(f'embedding shape{dataset_embedding.shape}')
    custom_dataset = Embedding_Dataset(dataset_embedding,dataset_target, ids, onehot)
...
def train_validate_model2(neural_network, train_dataloader, validation_dataloader, test_dataloader, device, optimizer, loss_function, epochs, batch_size=None):
    ...
    network_reduce = nn.Sequential(
            nn.Linear(3072, 20, bias=False),
            # nn.Dropout(p=0.5),
            nn.Flatten())

    # Optimizer keeps track of gradients
    network_reduce.to(device)
    ...
    for epoch in range(epochs):
        ...
        for (minib, batch) in enumerate(train_dataloader):
            ...
            embedding_reduce = network_reduce(embedding)
            # print(f'embedding_reduce shape {embedding_reduce.shape}')
            # print(f'onehot shape {onehot.shape}')
            onehot = onehot.view(16,-1)

            combined_tensor = t.cat([embedding_reduce, onehot], dim=1)

            predict = neural_network(combined_tensor)
...
def main():
    ...
    train_dataloader = create_data_loaders(df, train_emb, param.batch_size, param.num_workers)
    validation_dataloader = create_data_loaders(df, val_emb, param.batch_size, param.num_workers)
    test_dataloader = create_data_loaders(df_test, test_emb, param.batch_size, param.num_workers)
